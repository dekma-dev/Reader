...
const char* host = "192.168.199.208"; //server's ip-address
...
void loop() {
  delay(2000); //needed to avoid microcontroller overload 
  ...
  Serial.print("Requesting URL: "); //may be deleted
  Serial.println(url); //too
  ...
  if (Serial2.available() > 0) {
    ...
    url = "http://127.0.0.1:8000/monitoring/sending/?ID_stanok=124&RFID=SF234AS&Count=" + (String)count + "&State=1&Purpose=wqewqe&Country=qweqwe";
  ...
  // Serial.println("\n\nThe ID: " + (String)id_stanok + " and the datas: " + RFID + " " + (String)count);
  // Serial.println("request: ?id_stanok=" + (String)id_stanok + "&RFID=" + RFID + "&count=" + (String)count + "\n\n");
